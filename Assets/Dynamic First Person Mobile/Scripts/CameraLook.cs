...
namespace FirstPersonMobileTools.DynamicFirstPerson
{
	...
	public class CameraLook : MonoBehaviour
	{
		...
		private void Update()
		{
			if (Input.touchCount == 0 || InteractionManager.IsInteractionActive || Email_Manager.Email_Active) return;
			foreach (var touch in Input.touches)
			{
				if (touch.phase == TouchPhase.Began)
				{
					// Only add touches that did not start on a UI element.
					if (m_EventStytem != null && m_EventStytem.IsPointerOverGameObject(touch.fingerId))
						continue;
					if (m_AvailableTouchesId.Count <= m_TouchLimit)
						m_AvailableTouchesId.Add(touch.fingerId.ToString());
				}

				// Only process touches that began off UI.
				if (!m_AvailableTouchesId.Contains(touch.fingerId.ToString()))
					continue;
		...
		{
			if (InteractionManager.IsInteractionActive || Email_Manager.Email_Active) return;
